...
let data5 = {
  ...
  */
  ...
  imagesData.forEach((item, index) => { // ez teszi bele a képeket a boxokba
    ...
    $('.thumbnails-pictures').click((event) => {
      ...
      $('#next').click(() => {
        if (indexClicked < imagesData.length - 1) {  //  hogyha a current photo erteke nagyonn mint a tomb akkor ne menjen tovabb
          //akkor kell ha azt akarom h korbe menjen: indexClicked=0
          indexClicked++;
          loadPhoto(indexClicked)
          setCurrentThumbnailBorder(indexClicked); //beszinezi a kovetkezo thumbot
          console.log("This is the LAST item");
        }
        else if (indexClicked == 1) {
          setCurrentThumbnailBorder(indexClicked);
          console.log("This is the last item");
        }
      });
      ...
      let setCurrentThumbnailBorder = (indexClicked) => {
        if (indexClicked == 0) {  //a clickelt thumb legyen szines
          $('#0').css("border", "3px ridge #946e08");
          $('#1, #2, #3, #4').css('border', borderdefault);
...
$("#next").click(() => {
  if (currentPhoto < imagesData.length - 1) {
    $("#" + currentPhoto).css("border", borderdefault);
    currentPhoto++;
    loadPhoto(currentPhoto);
    $("#" + currentPhoto).css("border", borderCss);
...
if (currentPhoto == 0) {  // defaultban az elso thumb legyen szines
  $('#0').css("border", "3px ridge #946e08");
  $('#1, #2, #3, #4').css('border', borderdefault);
}
                        ...
                        /* $('#next').click(() => {
                            if(currentPhoto<imagesData.length-1 ){  
                              ...
                              loadPhoto(currentPhoto)
                              console.log("This is the LAST item");
                            } if(currentPhoto == 0){  //a clickelt thumb legyen szines
                              $('#0').css("border", "3px ridge #946e08");
                              $('#1, #2, #3, #4').css('border', borderdefault);
